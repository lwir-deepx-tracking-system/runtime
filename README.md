...
새 WSL 사용자에게 `docker version`이 `/var/run/docker.sock` 접근 거부를 표시할 수 있습니다. 소켓아니이 `root:docker` 소유이고 사용자가 `docker` 그룹에 없다면, Ubuntu 셸에서 다음 명령을 한 줄씩 실행합니다. `docker` 그룹은 Docker 엔진에 대한 높은 권한을 부여합니다. [Docker의 사용자 권한 안내](https://docs.docker.com/engine/install/linux-postinstall/)
...
mkdir -p ~/lwir-work
cd ~/lwir-work
git clone https://github.com/seu20/lwir-npu-tracking-system.git
cd lwir-npu-tracking-system
```
...
2. Windows에 VS Code와 Microsoft의 [WSL 확장](https://code.visualstudio.com/docs/remote/wsl)을 한 번 설치합니다. Ubuntu 셸에서 프로젝트를 엽니다.
   ...
   ```bash
   cd ~/lwir-work/lwir-npu-tracking-system
   code .
   ```

   VS Code 왼쪽 아래에 `WSL: Ubuntu-22.04`가 표시되는지 확인합니다. 이후 **Terminal → New Terminal**을 열면 WSL 터미널이 열립니다. 터미널의 현재 위치가 `/home/<사용자명>/lwir-work/lwir-npu-tracking-system`인지 `pwd`로 확인할 수 있습니다. 저장소는 Windows의 `C:\...`가 아니라 WSL의 `/home/...`에 있습니다.
...
4. VS Code에서 코드를 수정합니다. WSL 터미널의 **프로젝트 저장소 루트**에서 컨테이너를 엽니다.
   ...
   `root@...:/workspace/lwir-npu-tracking-system/runtime#`처럼 프롬프트가 바뀌면 컨테이너 안입니다. 여기에서 빌드하고 실행합니다.
